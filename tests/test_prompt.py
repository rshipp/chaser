...
class TestPrompt(unittest.TestCase):
    ...
    def test_correct_lower_upper(self):
        def user_input1(x):
            self.assertEquals(prompt.YES.upper(), x[2])
            self.assertEquals(prompt.NO.lower(), x[4])
            return prompt.YES
        prompt.user_input = user_input1
        r = prompt.prompt("")

        def user_input2(x):
            self.assertEquals(prompt.YES.lower(), x[2])
            self.assertEquals(prompt.NO.upper(), x[4])
            return prompt.YES
